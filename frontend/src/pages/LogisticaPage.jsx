...
import {
    ...
    FaUserTie, FaHistory, FaClock, FaBoxOpen, FaEdit,
    FaSave, FaEraser, FaExclamationTriangle, FaChevronLeft, FaChevronRight, FaPaperPlane, FaInfoCircle
} from "react-icons/fa";
...
export default function LogisticaPage() {
    ...
    const fetchDespachos = async () => {
        setLoading(true);
        try {
            const res = await fetch(`${API_BASE_URL}/logistica/despachos-automaticos`);
    ...
    useEffect(() => {
        ...
        if (activeTab === "DESPACHOS") fetchDespachos(); // <--- NUEVO
    }, [activeTab]);
    ...
    const filtrarNegativos = () => {
        const negativos = stockGlobal.filter(item =>
    ...
    const guardarEdicion = async (id) => {
        try {
            ...
            if (res.ok) {
                const updater = (prev) => prev.map(item => item.id === id ? {
                    ...item,
    ...
    const handleGenerarRemito = async () => {
        ...
        try {
            ...
            if (data.success) {
                ...
                setRemitoData({
                    codigo: data.codigo_remito, qrUrl: url, items: [...carrito],
                    origen: formEnvio.origen, destino: formEnvio.destino, chofer: formEnvio.chofer,
        ...
        <div className="max-w-7xl mx-auto animate-in fade-in pb-20">
            <div className="flex items-center gap-4 mb-8">
                ...
                <div>
                    ...
                    <p className="text-gray-400">Control de Stock: P26, P37, Ayolas y Quintana.</p>
                </div>
            </div>
            ...
            <div className="flex border-b border-slate-700 mb-6 overflow-x-auto">
                ...
                <Tab icon={<FaPaperPlane />} label="Despachos Auto" active={activeTab === "DESPACHOS"} onClick={() => setActiveTab("DESPACHOS")} />
            </div>
                ...
                <div className="space-y-6">
                    <div className="bg-slate-800 p-4 rounded-xl border border-slate-700 flex flex-col md:flex-row gap-4 items-center">
                        <div className="flex-1 w-full">
                            <AutoCompleteInput key={autoCompleteReset} items={stockGlobal} onSelect={agregarAStockVisible} placeholder="🔍 Buscar producto..." />
                        </div>
                        ...
                        <button onClick={() => setStockVisible([])} className="whitespace-nowrap px-4 py-3 bg-slate-700 hover:bg-slate-600 text-gray-300 rounded-lg font-bold flex items-center gap-2 text-sm"><FaEraser /> Limpiar</button>
                        <button onClick={filtrarNegativos} className="whitespace-nowrap px-4 py-3 bg-red-900/40 hover:bg-red-900/60 text-red-400 border border-red-900/50 rounded-lg font-bold flex items-center gap-2 text-sm"><FaExclamationTriangle /> Auditar Negativos</button>
                    </div>
                        ...
                        <table className="w-full text-left text-sm">
                            <thead className="bg-slate-900 text-gray-400 uppercase font-bold">
                                <tr>
                                    ...
                                    <th className="p-4 text-center text-purple-400 w-24">P. 37</th>
                                    <th className="p-4 text-center text-orange-400 w-24">Ayolas</th>
                                    <th className="p-4 text-center text-teal-400 w-24">Quintana</th>
                                    <th className="p-4 text-center text-white w-24 bg-white/5">TOTAL</th>
                                    <th className="p-4 text-right w-20"></th>
                                </tr>
                            </thead>
                            <tbody className="divide-y divide-slate-700">
                                {stockVisible.length === 0 ? (<tr><td colSpan="7" className="p-10 text-center text-gray-500 italic">Lista vacía.</td></tr>) : (
                                    stockVisible.map(item => {
                                        const isEditing = editRowId === item.id;
                                        const total = Number(isEditing ? editValues.p26 : item.stock_planta_26) +
                                            Number(isEditing ? editValues.p37 : item.stock_planta_37) +
                                            Number(isEditing ? editValues.ayolas : item.stock_deposito_ayolas) +
                                            Number(isEditing ? editValues.quintana : item.stock_deposito_quintana);
                                        const checkNeg = (val) => Number(val) < 0 ? "text-red-500 font-extrabold bg-red-900/20 px-2 py-1 rounded" : "font-bold";

                                        return (
                                            <tr key={item.id} className={`hover:bg-slate-700/50 transition-colors ${isEditing ? "bg-slate-700/80" : ""}`}>
                                                <td className="p-4 font-medium text-white"><div className="flex flex-col"><span>{item.nombre}</span><span className="text-xs text-gray-500 font-mono">{item.codigo}</span></div></td>
                                                <td className="p-4 text-center">{isEditing ? <input type="number" className="w-full bg-slate-900 border border-blue-500 rounded p-1 text-center text-white font-bold" value={editValues.p26} onChange={e => setEditValues({ ...editValues, p26: e.target.value })} /> : <span className={checkNeg(item.stock_planta_26)}>{item.stock_planta_26}</span>}</td>
                                                <td className="p-4 text-center">{isEditing ? <input type="number" className="w-full bg-slate-900 border border-purple-500 rounded p-1 text-center text-white font-bold" value={editValues.p37} onChange={e => setEditValues({ ...editValues, p37: e.target.value })} /> : <span className={checkNeg(item.stock_planta_37)}>{item.stock_planta_37}</span>}</td>
                                                <td className="p-4 text-center">{isEditing ? <input type="number" className="w-full bg-slate-900 border border-orange-500 rounded p-1 text-center text-white font-bold" value={editValues.ayolas} onChange={e => setEditValues({ ...editValues, ayolas: e.target.value })} /> : <span className={checkNeg(item.stock_deposito_ayolas)}>{item.stock_deposito_ayolas}</span>}</td>
                                                <td className="p-4 text-center">{isEditing ? <input type="number" className="w-full bg-slate-900 border border-teal-500 rounded p-1 text-center text-white font-bold" value={editValues.quintana} onChange={e => setEditValues({ ...editValues, quintana: e.target.value })} /> : <span className={checkNeg(item.stock_deposito_quintana)}>{item.stock_deposito_quintana}</span>}</td>
                                                <td className="p-4 text-center font-black text-white bg-white/5">{total}</td>
                                                <td className="p-4 text-right">
                                                    {isEditing ? (
                                                        <div className="flex justify-end gap-2"><button onClick={() => guardarEdicion(item.id)} className="p-2 bg-green-600 text-white rounded"><FaSave /></button><button onClick={cancelarEdicion} className="p-2 bg-gray-600 text-white rounded"><FaTimes /></button></div>
                                                    ) : (
                                                        <div className="flex justify-end gap-2"><button onClick={() => iniciarEdicion(item)} className="p-2 text-blue-400 hover:bg-slate-700 rounded"><FaEdit /></button><button onClick={() => quitarDeStockVisible(item.id)} className="p-2 text-gray-500 hover:bg-slate-700 rounded"><FaTimes /></button></div>
                                                    )}
                                                </td>
                                            </tr>
                                        );
                    ...
                    <div className="space-y-6">
                            ...
                            <div className="flex gap-4 items-end">
                                <div className="flex-1">
                                    ...
                                    <select className="w-full bg-slate-900 border-slate-600 rounded p-2 text-white" value={formEnvio.origen} onChange={e => setFormEnvio({ ...formEnvio, origen: e.target.value })}>
                                        <option value="PLANTA_26">Planta 26</option><option value="PLANTA_37">Planta 37</option><option value="DEP_AYOLAS">Ayolas</option><option value="DEP_QUINTANA">Quintana</option>
                                    </select>
                                </div>
                                ...
                                <div className="flex-1">
                                    ...
                                    <select className="w-full bg-slate-900 border-slate-600 rounded p-2 text-white" value={formEnvio.destino} onChange={e => setFormEnvio({ ...formEnvio, destino: e.target.value })}>
                                        <option value="DEP_AYOLAS">Ayolas</option><option value="DEP_QUINTANA">Quintana</option><option value="PLANTA_26">Planta 26</option><option value="PLANTA_37">Planta 37</option>
                                    </select>
                                </div>
                            </div>
                            <div><label className="text-xs uppercase text-gray-400 font-bold block mb-1 flex items-center gap-1"><FaUserTie /> Chofer</label><input type="text" placeholder="Nombre del conductor..." className="w-full bg-slate-900 border border-slate-600 rounded p-2 text-white focus:border-blue-500 outline-none" value={formEnvio.chofer} onChange={e => setFormEnvio({ ...formEnvio, chofer: e.target.value })} /></div>
                        </div>
                        <div className="bg-slate-800 p-5 rounded-xl border border-slate-700 min-h-[300px]">
                            ...
                            {carrito.length === 0 ? <div className="text-center text-gray-500 py-10 border-2 border-dashed border-slate-700 rounded-lg">Lista vacía.</div> :
                                <ul className="space-y-2">{carrito.map(item => (
                                    <li key={item.id} className="bg-slate-700 p-3 rounded-lg flex items-center justify-between"><div className="flex-1"><p className="text-white font-medium text-sm truncate">{item.nombre}</p><p className="text-xs text-gray-400">{item.codigo}</p></div><div className="flex items-center gap-3"><input type="number" min="1" value={item.cantidad} onChange={(e) => cambiarCantidad(item.id, e.target.value)} className="w-16 bg-slate-900 border border-slate-600 rounded p-1 text-center text-white font-bold" /><button onClick={() => quitarDelCarrito(item.id)} className="text-red-400"><FaTrash /></button></div></li>
                                ))}</ul>}
                        ...
                        {remitoData ? (
                            ...
                            <div className="bg-slate-800 p-5 rounded-xl border border-slate-700 h-full flex flex-col">
                                ...
                                <div className="overflow-y-auto flex-1 custom-scrollbar pr-2">
                                    {resultadosBusquedaEnvio.length === 0 ? <p className="text-center text-gray-500 mt-10">Sin resultados.</p> : <ul className="space-y-2">{resultadosBusquedaEnvio.map(item => {
                                        const enCarrito = carrito.some(c => c.id === item.id);
                                        return (<li key={item.id} className="flex items-center justify-between p-3 bg-slate-700/50 rounded-lg border border-slate-600/50 mb-2"><div><p className="text-white font-medium text-sm">{item.nombre}</p><p className="text-xs text-gray-400">Total: {Number(item.stock_planta_26) + Number(item.stock_planta_37) + Number(item.stock_deposito_ayolas) + Number(item.stock_deposito_quintana)}</p></div><button onClick={() => agregarAlCarrito(item)} disabled={enCarrito} className={`w-8 h-8 flex items-center justify-center rounded-full ${enCarrito ? "bg-green-600" : "bg-blue-600"}`}>{enCarrito ? <FaCheckCircle /> : <FaPlus />}</button></li>);
                                    })}</ul>}
                                </div>
                            </div>
                        )}
                ...
                <div className="flex flex-col items-center justify-center min-h-[400px]">
                    {!scanResult && !scanError ? (
                        <div className="w-full max-w-md bg-black rounded-2xl overflow-hidden shadow-2xl border-4 border-slate-700 relative">
                            ...
                            <p className="text-center text-white p-4 bg-slate-900 font-bold">Escanear QR de Remito</p>
                        </div>
                    ) : (
            ...
            {/* TAB DESPACHOS AUTOMÁTICOS */}
            {activeTab === "DESPACHOS" && (
                <div className="space-y-4">
                    <div className="bg-blue-900/20 border border-blue-500/30 p-4 rounded-xl flex items-center gap-3 text-blue-200">
                        <FaInfoCircle className="text-xl" />
                        <div>
                            <p className="font-bold text-sm">Sincronización Automática</p>
                            <p className="text-xs opacity-80">Estos movimientos se generan automáticamente cuando un pedido tiene "Fecha Despacho" en el Excel. Descuentan stock de DEPÓSITO QUINTANA.</p>
                        </div>
                    </div>
                        ...
                        <table className="w-full text-left text-sm">
                            <thead className="bg-slate-900 text-gray-400 uppercase font-bold">
                                <tr>
                                    ...
                                    <th className="p-4">Cliente / OC</th>
                                    <th className="p-4">Producto Terminado</th>
                                    <th className="p-4 text-purple-400">Semielaborado (Quintana)</th>
                                    <th className="p-4 text-right text-red-400">Cant. Desc.</th>
                                </tr>
                            </thead>
                            <tbody className="divide-y divide-slate-700">
                                {despachos.length === 0 ? (
                                    <tr><td colSpan="5" className="p-8 text-center text-gray-500">No hay despachos registrados aún.</td></tr>
                                ) : (
                                    despachos.map(d => (
                                        <tr key={d.id} className="hover:bg-slate-700/50">
                                            ...
                                            <td className="p-4">
                                                <div className="font-bold text-white">{d.cliente}</div>
                                                <div className="text-xs text-gray-500 font-mono">OC: {d.oc}</div>
                                            </td>
                                            <td className="p-4 text-gray-300">{d.modelo_producto}</td>
                                            <td className="p-4 text-purple-300 font-medium">{d.semielaborado_nombre}</td>
                                            <td className="p-4 text-right font-bold text-red-400">-{d.cantidad_descontada}</td>
                                        </tr>
                                    ))
                                )}
                            </tbody>
            ...
            {activeTab === "HISTORIAL" && (
                <div className="space-y-6">
                    <div className="grid grid-cols-1 gap-4">
                        {historial.length === 0 && <p className="text-center text-gray-500 py-10">No hay movimientos registrados.</p>}
                        {currentHistoryItems.map((viaje, i) => {
                            const isRecibido = viaje.estado === 'RECIBIDO';
                            return (
                                <div key={i} className="bg-slate-800 rounded-xl border border-slate-700 p-6 shadow-lg flex flex-col md:flex-row justify-between items-center gap-6 relative overflow-hidden">
                    ...
                    {totalHistoryPages > 1 && (
                        <div className="flex justify-center items-center gap-4 mt-8">
                            <button onClick={() => setHistoryPage(p => Math.max(1, p - 1))} disabled={historyPage === 1} className="p-3 bg-slate-800 text-white rounded-full hover:bg-slate-700 disabled:opacity-50"><FaChevronLeft /></button>
                            <span className="text-gray-400 text-sm font-bold">Página {historyPage} de {totalHistoryPages}</span>
                            <button onClick={() => setHistoryPage(p => Math.min(totalHistoryPages, p + 1))} disabled={historyPage === totalHistoryPages} className="p-3 bg-slate-800 text-white rounded-full hover:bg-slate-700 disabled:opacity-50"><FaChevronRight /></button>
                        </div>
                    )}
                </div>
            )}
