...
const imprimirSolicitud = (id, items, fechaStr) => {
  ...
  autoTable(doc, {
    ...
    columnStyles: {
      0: { halign: "center", fontStyle: "bold", width: 30 },
      2: { halign: "right", fontStyle: "bold", width: 20 },
      3: { fontStyle: "italic" },
    },
  });
  ...
  for (let i = 1; i <= pageCount; i++) {
    ...
    doc.text(
      `Página ${i} de ${pageCount} - Documento generado digitalmente`,
      105,
      290,
      { align: "center" }
    );
...
export default function SolicitudesPage() {
  ...
  const cargarDatos = async () => {
    try {
      ...
      const [resMP, resHist] = await Promise.all([
        ...
        fetch(`${API_BASE_URL}/compras/historial`),
      ]);
  ...
  const agregarItem = (mp) => {
    if (carrito.find((i) => i.id === mp.id)) return;
  ...
  const handleGenerarNueva = async () => {
    ...
    try {
      ...
      const res = await fetch(`${API_BASE_URL}/compras/nueva`, {
        method: "POST",
        headers: { "Content-Type": "application/json" },
  ...
  const reimprimirDesdeHistorial = async (e, solId, fecha) => {
    e.stopPropagation();
    try {
      const res = await fetch(`${API_BASE_URL}/compras/solicitud/${solId}`);
      if (res.ok) {
        const data = await res.json();
  ...
  return (
    <div className="max-w-6xl mx-auto animate-in fade-in pb-20">
      <div className="flex items-center gap-4 mb-8">
        <FaShoppingCart className="text-4xl text-green-400" />
        <div>
          <h1 className="text-3xl font-bold text-white">
            Solicitudes de Compra
          </h1>
          <p className="text-gray-400">
            Generar requisiciones y controlar recepción.
          </p>
        </div>
      </div>

      <div className="grid grid-cols-1 lg:grid-cols-3 gap-8 mb-12">
        <div className="bg-slate-800 p-5 rounded-xl border border-slate-700 col-span-1 h-fit">
          <h3 className="text-white font-bold mb-4 flex items-center gap-2">
            <FaSearch /> Buscar Material
          </h3>
          <AutoCompleteInput
        ...
        <div className="bg-slate-800 p-5 rounded-xl border border-slate-700 col-span-2 min-h-[300px] flex flex-col">
          <h3 className="text-white font-bold mb-4 flex items-center gap-2">
            <FaPlus /> Nueva Solicitud ({carrito.length})
          </h3>
          <div className="flex-1">
            {carrito.length === 0 ? (
              <div className="text-center text-gray-500 py-10 border-2 border-dashed border-slate-600 rounded-lg">
                Vacío.
              </div>
            ) : (
              <ul className="space-y-3">
                {carrito.map((item) => (
                  <li
                    key={item.id}
                    className="bg-slate-700 p-3 rounded-lg flex flex-col md:flex-row md:items-center gap-4 animate-in slide-in-from-left-2"
                  >
                    <div className="flex-1">
                    ...
                    <div className="flex items-center gap-2">
                      <input
                        type="number"
                        className="w-24 bg-slate-900 border border-slate-600 rounded p-2 text-white text-center font-bold"
                        placeholder="Cant."
                        value={item.cantidad}
                        onChange={(e) =>
                      ...
                      <input
                        ...
                        onChange={(e) =>
                          actualizarItem(item.id, "proveedor", e.target.value)
                        }
                      />
                      <button
                        onClick={() => eliminarItem(item.id)}
                        className="text-red-400 p-2 hover:bg-slate-600 rounded"
                      >
                        <FaTrash />
                      </button>
                    </div>
          ...
          {carrito.length > 0 && (
            <button
              ...
              className="w-full mt-6 bg-green-600 hover:bg-green-500 text-white font-bold py-3 rounded-lg shadow-lg flex items-center justify-center gap-2"
            >
              <FaFilePdf /> Generar y Descargar
            </button>
          )}
        </div>
      </div>
                ...
                >
                  ...
                  <td className="p-4 text-gray-300">
                    <div className="flex items-center gap-2">
                      ...
                      {new Date(sol.fecha_creacion).toLocaleDateString("es-AR")}
                    </div>
                  </td>
                  ...
                  <td className="p-4 text-right">
                    <button
                      onClick={(e) =>
                        reimprimirDesdeHistorial(e, sol.id, sol.fecha_creacion)
                      }
                      className="bg-slate-700 hover:bg-slate-600 text-white p-2 rounded shadow mr-2"
        ...
        {totalPages > 1 && (
          <div className="p-4 border-t border-slate-700 flex justify-center items-center gap-4">
            <button
              ...
              disabled={page === 1}
              className="p-2 bg-slate-700 text-white rounded disabled:opacity-50"
            >
              <FaChevronLeft />
            </button>
