...
router.use(protect);
router.use(restrictTo("GERENCIA", "JEFE PRODUCCIÓN", "ADMIN"));
  ...
  try {
    ...
    for (const item of items) {
      await client.query(
        ...
        [solId, item.materia_prima_id, item.cantidad, item.proveedor || "-"],
      );
    }
...
router.get("/solicitud/:id", protect, async (req, res) => {
  const { id } = req.params;
  const cab = await db.query("SELECT * FROM solicitudes_compra WHERE id = $1", [
    id,
  ]);
...
router.put("/item/:id", protect, async (req, res) => {
  ...
  try {
    ...
    await client.query(
      ...
      [cantidad, proveedor, req.params.id],
    );
    const itemRes = await client.query(
...
router.post("/solicitud/:id/items", protect, async (req, res) => {
  ...
  try {
    ...
    await client.query(
      "INSERT INTO solicitudes_items (solicitud_id, materia_prima_id, cantidad, proveedor_recomendado) VALUES ($1, $2, $3, $4)",
      [req.params.id, materia_prima_id, cantidad, proveedor],
    );
